...
class LocationQRPage extends StatefulWidget {
  const LocationQRPage({super.key});

  @override
  State<LocationQRPage> createState() => _LocationQRPageState();
}

class _LocationQRPageState extends State<LocationQRPage> {
  ...
  void _generateLocationQR() {
    if (_latitudeController.text.trim().isNotEmpty &&
        _longitudeController.text.trim().isNotEmpty) {
      String lat = _latitudeController.text.trim();
      String lng = _longitudeController.text.trim();
      String label = _labelController.text.trim();

      // Google Maps format
      String mapsUrl = 'https://maps.google.com/maps?q=$lat,$lng';
      if (label.isNotEmpty) {
        mapsUrl += '($label)';
      }
    ...
    } else if (_latitudeController.text.trim().isNotEmpty ||
        _longitudeController.text.trim().isNotEmpty) {
      setState(() {
        _qrData = 'Invalid location data';
        _showQR = false;
      });
    } else {
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 4),
                            Text(
                              'Label: ${_labelController.text.trim().isEmpty ? "-" : _labelController.text.trim()}',
                              style: TextStyle(
                                fontSize: 16,
                                color: Theme.of(context).colorScheme.onSurfaceVariant,
                                fontWeight: FontWeight.w500,
